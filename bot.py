...
async def notify_question(user_id: int, username: str, user_info: str, question: str):
    for admin_id in ADMIN_IDS:
        await bot.send_message(
            admin_id,
            f"❓ НОВЫЙ ВОПРОС\n\n👤 @{username} (ID: {user_id})\n📝 {user_info}\n💬 {question}\n\n✏️ Чтобы ответить, отправьте:\n/answer {user_id} Ваш ответ здесь"
        )
...
async def handle_question(message: types.Message, state: FSMContext):
    ...
    # Если пользователь нажал кнопку - обрабатываем как ответ
    if text in ["✅ Да, готов(а) попробовать", "❓ Есть вопросы", "❌ Нет, не подходит"]:
        if text == "✅ Да, готов(а) попробовать":
            ...
            await message.answer(
                "🙏 **Спасибо за честность!**\n\n"
                "Если в будущем передумаете — возвращайтесь.\n\n"
                "🍀 **Удачи!**",
                reply_markup=ReplyKeyboardRemove()
            )
            await state.clear()
